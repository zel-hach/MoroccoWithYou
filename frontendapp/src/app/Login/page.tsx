...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await api.post<{ user: { id: number; email: string; name?: string | null } }>(
        "/users/login",
        { email, password },
      );
      const u = response.data?.user;
      if (u?.id && u?.email) {
        setStoredUser({ id: u.id, email: u.email, name: u.name ?? null });
  ...
  return (
    <>
      ...
      <div className="absolute inset-0 z-0" aria-hidden>
          ...
          <form className="flex h-full w-full flex-col items-center justify-center gap-10 p-10" onSubmit={handleLogin}>
          <h1 className="mt-20 text-4xl">Let’s start your Journey together! </h1>
          {error ? <p className="max-w-md rounded-lg border border-red-500/40 bg-red-500/10 px-4 py-2 text-center text-sm text-red-200">{error}</p> : null}
